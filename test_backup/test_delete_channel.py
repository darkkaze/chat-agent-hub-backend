...
async def test_delete_channel_not_found(session):
    ...
    try:
        result = await delete_channel(channel_id="nonexistent_channel", token=token, db_session=session)
...
async def test_delete_channel_with_agent_associations(session):
    ...
    agent1 = Agent(
        name="Agent 1",
        webhook_url="https://agent1.example.com/callback"
    )
    
    agent2 = Agent(
        name="Agent 2", 
        webhook_url="https://agent2.example.com/callback"
    )
